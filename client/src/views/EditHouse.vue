...
export default {
  ...
  setup() {
    ...
    const checkAuth = () => {
      ...
      if (!token) {
        // If no token, redirect to login and stop further execution
        router.push('/login');
        return false;
      }
        ...
        if (err.response) {
          switch (err.response.status) {
            case 401:
              ...
              break;
            case 403:
              error.value = 'You do not have permission to edit this house';
              break;
            case 404:
    ...
    const handleSubmit = async () => {
      // Validate authentication and form
        ...
        if (err.response) {
          switch (err.response.status) {
            case 401:
              ...
              break;
            case 403:
              submitError.value = 'You do not have permission to edit this house';
              break;
            case 400:
              submitError.value = err.response.data?.message || 'Invalid input. Please check your details.';
              break;
            default:
